        ...
        <div class="col-lg-9">
            <div class="row">
                ...
                <div class="container">
                    ...
                    <a href="/admin/products/add" style="margin: 20px 0" class="btn btn-outline-success">Add</a>
                    <table class="table">
                        <thead class="thead-light">
                        <tr>
                            <th scope="col">STT</th>
                            <th scope="col"><a href="/admin/products?sort=name">Name</a></th>
                            <th scope="col">Image</th>
                            <th scope="col"><a href="/admin/products?sort=price">Price</a></th>
                            <th scope="col"><a href="/admin/products?sort=createDate">Create Date</a></th>
                        ...
                        <c:forEach var="product" items="${listProduct}" varStatus="status">
                            <tr>
                                ...
                                <td>
                                    <img width="80" height="80" src="/productImages/${product.image}" alt="Product Image">
                                </td>
                                <td><c:out value="${product.price}"/></td>
                                <td><c:out value="${product.createDate}"/></td>
                                <td><c:out value="${product.available ? 'native' : 'natived'}"/></td>
                                <td><c:out value="${product.category.name}"/></td>
                                <td>
                                    <a href="<c:url value='/admin/products/delete/${product.id}'/>" class="btn btn-outline-danger">Delete</a>
                                    <a href="<c:url value='/admin/products/update/${product.id}'/>" class="btn btn-outline-info">Update</a>
                                </td>
                            </tr>
                        </c:forEach>
                    ...
                    <div class="pagination">
                        <a href="/admin/products?p=0&sort=${param.sort}" class="${page.first ? 'disabled' : ''}">First</a>
                        <a href="/admin/products?p=${page.number - 1}&sort=${param.sort}" class="${page.first ? 'disabled' : ''}">Previous</a>

                        <c:forEach begin="0" end="${page.totalPages - 1}" var="i">
                            <a href="/admin/products?p=${i}&sort=${param.sort}" class="${i == page.number ? 'active' : ''}">${i + 1}</a>
                        </c:forEach>

                        <a href="/admin/products?p=${page.number + 1}&sort=${param.sort}" class="${page.last ? 'disabled' : ''}">Next</a>
                        <a href="/admin/products?p=${page.totalPages - 1}&sort=${param.sort}" class="${page.last ? 'disabled' : ''}">Last</a>
                    </div>
                </div>
