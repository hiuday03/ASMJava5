        ...
        <div class="col-lg-9">
            <div class="row">
                <div class="container">
                    ...
                    <a href="/admin/accounts/add" style="margin: 20px 0" class="btn  btn-outline-success">Add</a>
...
<%--                            <th scope="col">Photo</th>--%>
                            <th scope="col"><a href="/admin/accounts?sort=username">Username</a></th>
                            <th scope="col">Password</th>
                            <th scope="col"><a href="/admin/accounts?sort=fullname">Fullname</a></th>
                            <th scope="col"><a href="/admin/accounts?sort=email">Email</a></th>
<%--                            <th scope="col">Active</th>--%>
                        ...
                        <tbody>
                        <c:forEach items="${listAccount}" var="account" varStatus="iStat">
                            <tr>
                                <th scope="row"><c:out value="${iStat.index + 1}"/></th>
<%--                                <td>--%>
<%--                                    <img width="80" height="80" src="/categoryImages/${account.photo}"--%>
<%--                                         alt="Product Image">--%>
<%--                                </td>--%>
...
<%--                                <td><c:out value="${account.active ? 'native' : 'natived'}"/></td>--%>
                                <td><c:out value="${account.admin ? 'Admin' : 'Customer'}"/></td>
                                <td>
                                    <a href="/admin/accounts/delete/${account.username}" class="btn btn-outline-danger">Delete</a>
                                    <a style="margin-top: 5px" href="/admin/accounts/update/${account.username}"
                    ...
                    <div class="pagination">
                        <a href="/admin/accounts?p=0&sort=${param.sort}"
                           class="${page.first ? 'disabled' : ''}">First</a>
                        <a href="/admin/accounts?p=${page.number - 1}&sort=${param.sort}"
                           class="${page.first ? 'disabled' : ''}">Previous</a>

                        <c:forEach begin="0" end="${page.totalPages - 1}" var="i">
                            <a href="/admin/accounts?p=${i}&sort=${param.sort}"
                               class="${i == page.number ? 'active' : ''}">${i + 1}</a>
                        </c:forEach>

                        <a href="/admin/accounts?p=${page.number + 1}&sort=${param.sort}"
                           class="${page.last ? 'disabled' : ''}">Next</a>
                        <a href="/admin/accounts?p=${page.totalPages - 1}&sort=${param.sort}"
